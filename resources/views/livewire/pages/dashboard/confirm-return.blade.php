                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="w-9 h-12 bg-slate-50 border border-[#E2E8F0] rounded overflow-hidden flex items-center justify-center shrink-0">
                                                @if(!empty($item['cover_image']))
                                                    <img src="{{ $item['cover_image'] }}" alt="Cover" class="w-full h-full object-cover" onerror="this.onerror=null; this.src=''; this.parentElement.innerHTML='<div class=\'w-full h-full bg-gradient-to-br from-[#102B70] to-[#FCC719] flex items-center justify-center\'><span class=\'text-[8px] text-white font-bold\'>BOOK</span></div>';">
                                                @else
                                                    <div class="w-full h-full bg-gradient-to-br from-[#102B70] to-[#FCC719] flex items-center justify-center">
                                                        <span class="text-[8px] text-white/90 font-bold uppercase tracking-wider text-center px-1 leading-tight">{{ $item['accession'] }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                            <div class="flex flex-col min-w-0">
                                                <span class="font-bold text-[13.5px] leading-tight text-[#0F172A] truncate">{{ $item['title'] }}</span>
                                                <span class="text-[11px] text-[#64748B] mt-0.5 truncate">{{ $item['author'] }}</span>
                                                <span class="text-[10px] text-[#94A3B8] mt-1 font-semibold">Accession: {{ $item['accession'] }}</span>
                                            </div>
                                        </div>
                                    </td>
                                        ...
                                        @if($item['fine_amount'] > 0)
                                            <div class="flex flex-col items-end gap-1">
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-bold bg-[#FEF2F2] text-[#B91C1C] border border-[#FECACA]">
                                                    {{ $item['overdue_days'] }} {{ Str::plural('Day', $item['overdue_days']) }} Overdue
                                                </span>
