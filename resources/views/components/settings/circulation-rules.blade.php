    ...
    <div class="mt-8">
        <div class="flex items-center gap-3 mb-4">
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">Maximum number of active loans allowed per account type.</p>
            </div>
            @if($this->dirtyState['sections']['circulation.borrowing_limits'] ?? false)
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @foreach($settings['circulation']['borrowing_limits'] as $type => $limit)
                <div class="flex items-center justify-between p-4 rounded-xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <input type="number" wire:model.live="settings.circulation.borrowing_limits.{{ $type }}" min="1" max="50" class="h-9 w-20 rounded-md border border-slate-200 bg-slate-50 px-3 text-sm text-slate-900 focus:border-[#17357A] focus:bg-white focus:ring-[3px] focus:ring-[#17357A]/10 transition-shadow outline-none text-center">
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div>
        <div class="flex items-center gap-3 mb-4">
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">Standard borrowing periods by item category.</p>
            </div>
            @if($this->dirtyState['sections']['circulation.loan_durations'] ?? false)
                <div class="flex items-center gap-1.5 text-xs font-semibold text-amber-700 bg-amber-50 px-2 py-1 rounded border border-amber-200">
                    <span class="h-1.5 w-1.5 rounded-full bg-amber-500"></span>
                    Modified
                </div>
                    ...
                    @foreach($settings['circulation']['loan_durations'] as $category => $days)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-5 py-4 text-right">
                                <input type="number" wire:model.live="settings.circulation.loan_durations.{{ $category }}" min="1" max="90" class="h-9 w-20 rounded-md border border-slate-200 bg-slate-50 px-3 text-sm text-slate-900 focus:border-[#17357A] focus:bg-white focus:ring-[3px] focus:ring-[#17357A]/10 transition-shadow outline-none text-center inline-block">
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            ...
            @foreach($settings['circulation']['loan_durations'] as $category => $days)
                <div class="flex items-center justify-between p-4 rounded-xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" wire:model.live="settings.circulation.loan_durations.{{ $category }}" min="1" max="90" class="h-9 w-16 rounded-md border border-slate-200 bg-slate-50 px-2 text-sm text-slate-900 focus:border-[#17357A] focus:bg-white focus:ring-[3px] focus:ring-[#17357A]/10 transition-shadow outline-none text-center">
    ...
    <div>
        <div class="flex items-center gap-3 mb-4">
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">Configure overdue penalties charged to patrons. These settings apply only to patron overdue fines. They do not represent library operating or administrative costs.</p>
            </div>
            @if($this->dirtyState['sections']['circulation.fine_rules'] ?? false)
                <div class="flex items-center gap-1.5 text-xs font-semibold text-amber-700 bg-amber-50 px-2 py-1 rounded border border-amber-200">
                    <span class="h-1.5 w-1.5 rounded-full bg-amber-500"></span>
                    Modified
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" step="0.50" min="0" wire:model.live="settings.circulation.fine_rules.daily_fine" class="h-10 w-full rounded-lg border border-slate-200 bg-white pl-8 pr-3 text-sm text-slate-900 focus:border-[#17357A] focus:ring-[3px] focus:ring-[#17357A]/10 transition-shadow outline-none">
                </div>
    ...
    <div class="mb-2">
        <div class="flex items-center gap-3 mb-4">
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">Rules governing the extension of active loans.</p>
            </div>
            @if($this->dirtyState['sections']['circulation.renewal_limits'] ?? false)
                <div class="flex items-center gap-1.5 text-xs font-semibold text-amber-700 bg-amber-50 px-2 py-1 rounded border border-amber-200">
                    <span class="h-1.5 w-1.5 rounded-full bg-amber-500"></span>
                    Modified
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="number" min="0" wire:model.live="settings.circulation.renewal_limits.max_consecutive" class="h-10 w-24 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 focus:border-[#17357A] focus:ring-[3px] focus:ring-[#17357A]/10 transition-shadow outline-none text-center">
