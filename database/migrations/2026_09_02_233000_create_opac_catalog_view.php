...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if ($driver === 'pgsql') {
            ...
            DB::statement($this->createSqliteViewSql());
        } else {
...
SQL;
    ...
    /**
     * SQLite syntax for opac_catalog_view.
     */
    private function createSqliteViewSql(): string
    {
        return <<<SQL
CREATE VIEW IF NOT EXISTS opac_catalog_view AS
WITH copy_stats AS (
    SELECT 
        b.book_detail_id,
        COUNT(b.id) AS total_copies,
...
author_agg AS (
    SELECT 
        bda.book_data_id,
        GROUP_CONCAT(TRIM(a.first_name || ' ' || a.last_name), ', ') AS authors
    FROM book_data_author bda
    JOIN authors a ON a.id = bda.author_id AND a.deleted_at IS NULL
    GROUP BY bda.book_data_id
),
